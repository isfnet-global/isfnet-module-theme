...
as required by TPNewsSticker Module for http://www.templateplazza.com
*/
...
var QScroller = new Class({
	...
	load: function() {
		...
		var chScroll = this.options.hScroll;
		var cvScroll = this.options.vScroll;
		if(this.step==-1) {
			if(this.options.hScroll == 'left') {
				this.options.hScroll = 'right';
			} else {
				this.options.hScroll = 'left';
			}
			if(this.options.vScroll == 'top') {
				this.options.vScroll = 'bottom';
			} else {
				this.options.vScroll = 'top';
			}
		}
		this.show();
		this.options.hScroll = chScroll;
		this.options.vScroll = cvScroll;
	},
	show: function() {
